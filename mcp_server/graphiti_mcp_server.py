...
async def initialize_graphiti():
    ...
    logger.info(f'Starting Graphiti initialization with {config.database.db_type} database...')
    ...
    try:
        ...
        if config.database.db_type == 'neo4j':
            ...
            falkor_connection_string = os.getenv('FALKORDB_CONNECTION_STRING')
            if not falkor_connection_string:
                logger.warning(
                    'FALKORDB_CONNECTION_STRING not set, using default: redis://localhost:6379. '
                    'Set FALKORDB_CONNECTION_STRING for custom FalkorDB connection.'
                )
            else:
                logger.info(f'Using FalkorDB connection string: {falkor_connection_string}')
        ...
        # For FalkorDB, environment variables will be used automatically by the driver
        graphiti_kwargs = {
        ...
        if config.database.db_type == 'neo4j':
            ...
            # For FalkorDB, the driver will automatically use environment variables
            # FALKORDB_CONNECTION_STRING or FALKORDB_HOST/PORT/DATABASE/PASSWORD
            pass
        ...
        if config.database.db_type == 'falkordb':
            try:
                ...
                if node_count and node_count[0].get('count', 0) == 0:
                    logger.debug('Empty FalkorDB graph detected, using minimal indexing')
        ...
        logger.info(f'Graphiti client initialized successfully in {total_init_time:.2f}s')
        ...
        if embedder_client:
            ...
            logger.info('No embedder client configured - semantic search will be limited')
        ...
        if config.database.db_type == 'falkordb':
            ...
            if total_init_time > 5.0:
                ...
                logger.info(f'✅ FalkorDB startup target achieved: {total_init_time:.2f}s < 5s')
        ...
        if db_type == 'falkordb':
            logger.error(
                ...
                f'Ensure FalkorDB is running at {os.getenv("FALKORDB_CONNECTION_STRING", "redis://localhost:6379")}. '
            ...
            logger.error(f'Database connection refused after {init_time:.2f}s: {str(e)}')
        raise
        ...
        if 'timeout' in str(e).lower():
            if db_type == 'falkordb':
                ...
                logger.error('Hint: Neo4j connection timeout. Check if Neo4j server is responsive.')
    ...
    try:
        ...
        if config.database.db_type == 'neo4j':
            ...
            try:
                ...
                raise ConnectionError(f'FalkorDB connection test failed: {str(e)}') from e
...
async def get_performance_metrics() -> StatusResponse:
    ...
    try:
        ...
        try:
            ...
            metrics['memory_note'] = 'Install psutil for memory monitoring: pip install psutil'
        except Exception as e:
        ...
        if config.database.db_type == 'falkordb':
            ...
            message = f'FalkorDB Performance: Memory {memory_status} Query {query_status}'
        else:
