...
class TestDriverFactory:
    ...
    def test_create_driver_with_config_parameter(self, mock_neo4j_driver_class):
        ...
        config = {"some": "config"}
        driver = DriverFactory.create_driver(
            "bolt://localhost:7687", "neo4j", "password", config=config
        )

        # Config should be accepted but not used (database should be None since not provided)
        mock_neo4j_driver_class.assert_called_once_with(
            "bolt://localhost:7687", "neo4j", "password"
        )
        assert driver is mock_driver
    ...
    @patch.dict(os.environ, {"GRAPHITI_DB_TYPE": "neo4j"})
    @patch("graphiti_core.driver.factory.Neo4jDriver")
    def test_create_driver_with_database_parameter(self, mock_neo4j_driver_class):
        """Test that factory passes database parameter to Neo4j driver."""
        mock_driver = MagicMock(spec=GraphDriver)
        mock_neo4j_driver_class.return_value = mock_driver

        driver = DriverFactory.create_driver(
            "bolt://localhost:7687", "neo4j", "password", database="test_db"
        )

        # Database parameter should be passed through
        mock_neo4j_driver_class.assert_called_once_with(
            "bolt://localhost:7687", "neo4j", "password", database="test_db"
        )
        assert driver is mock_driver
    ...
    @patch.dict(os.environ, {"GRAPHITI_DB_TYPE": "falkordb"})
    def test_create_driver_falkordb_with_database_parameter(self):
        """Test that factory passes database parameter to FalkorDB driver."""
        with patch(
            "graphiti_core.driver.falkordb_driver.FalkorDriver"
        ) as mock_falkor_driver_class:
            mock_driver = MagicMock(spec=GraphDriver)
            ...
            driver = DriverFactory.create_driver(database="test_db")

            # Database parameter should be passed through
            mock_falkor_driver_class.assert_called_once_with(database="test_db")
            assert driver is mock_driver
