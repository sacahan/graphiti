...
class TestFalkorDBConfig:
    ...
    def test_custom_values(self):
        ...
        config = FalkorDBConfig(
            host="custom-host", port=9999, database=5, password="secret"
        )
    ...
    @unittest.skipIf(not HAS_FALKORDB, "FalkorDB is not installed")
    @patch.dict(
        os.environ, {"FALKORDB_CONNECTION_STRING": "redis://:secret@remote-host:8000/5"}
    )
    ...
    def test_from_connection_string_with_password(self):
        ...
        config = FalkorDBConfig.from_connection_string(
            "redis://:mypassword@example.com:9999/3"
        )
...
class TestFalkorDriverWithConfig:
    ...
    def test_driver_legacy_username_parameter(self, mock_falkor_db_class):
        ...
        driver = FalkorDriver(username="legacy-user")
    ...
    def test_driver_password_overrides_username(self, mock_falkor_db_class):
        ...
        driver = FalkorDriver(username="legacy-user", password="actual-password")
